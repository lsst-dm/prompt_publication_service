...
class Dataset(Base):
    ...
    embargo_status: Mapped[DatasetLocationStatus] = mapped_column(_EnumColumn(DatasetOrigin))
    prompt_prep_status: Mapped[DatasetLocationStatus] = mapped_column(
        _EnumColumn(DatasetOrigin), default=DatasetLocationStatus.NEVER_PRESENT
    )
    ...
    __table_args__ = (
        ...
        *(
            Index(f"{column}_lookup", "dataset_type", column, "prompt_prep_status")
            for column in (
                "embargo_status",
                "repo_main_status",
                "google_prod_status",
